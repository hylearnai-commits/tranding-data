...
def verify_api_key_and_rate_limit(x_api_key: str | None = Header(default=None)):
    ...
    if not configured_keys:
        raise_api_error(code="INTERNAL_ERROR", message="AUTH_ENABLED=true 但未配置API_KEYS")
    if not x_api_key or x_api_key not in configured_keys:
        raise_api_error(code="UNAUTHORIZED", message="无效的API Key")
    now = time()
    window_start = now - 60
    with _rate_limit_lock:
        ...
        if len(q) >= settings.rate_limit_per_minute:
            raise_api_error(code="RATE_LIMITED", message="请求过于频繁，请稍后再试")
        q.append(now)
